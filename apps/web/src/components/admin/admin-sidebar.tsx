...
import {
  ...
  AvatarFallback,
  AvatarImage,
} from "@echoray/ui/components/ui/avatar";
...
export function AdminSidebar({
  ...
  return (
    <Sidebar
      collapsible="icon"
      className="border-r border-border bg-zinc-950"
      {...props}
    >
            ...
            return (
              <SidebarMenuItem key={item.href}>
                ...
                >
                  <Link href={item.href}>
                    <item.icon
                      className={cn("size-4", isActive ? "text-blue-400" : "")}
                    />
          ...
          <SidebarMenuItem className="flex items-center gap-2 p-2 rounded-xl bg-white/5 border border-white/5">
            ...
            {!isCollapsed && (
              <div className="grid flex-1 text-left text-sm leading-tight ml-1">
                ...
                <span className="truncate text-[10px] uppercase text-blue-400 font-black">
                  Admin
                </span>
              </div>
            )}
