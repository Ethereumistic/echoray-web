...
// Example Cloudflare Worker API endpoint with permission checking
...
interface Env {
  ...
  PERMISSIONS_KV?: KVNamespace; // Optional
}
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === '/api/analytics/export') {
      return handleAnalyticsExport(request, env, supabase);
    }
...
async function handleAnalyticsExport(
  ...
  // 2. Set auth token in Supabase client
  ...
  // 4. Get organization from request (could be query param, header, etc.)
  const url = new URL(request.url);
  ...
  // 5. Check permission (this is the key part!)
  const { data: hasPermission, error: permError } = await supabase.rpc(
    'user_has_permission',
    {
      p_user_id: user.id,
      p_organization_id: organizationId,
      p_permission_code: 'export.csv'
    }
  );
  ...
  // 6. User has permission, proceed with actual logic
...
// Helper function to require permission (reusable)
...
// Example: Middleware-style helper
export async function withPermission(
  ...
  const { allowed, error } = await requirePermission(
    supabase,
    user.id,
    organizationId,
    permissionCode
...
```typescript
// functions/api/members/invite.ts
// Cloudflare Pages Function with permission check
...
export async function onRequestPost(context) {
  ...
  // Parse request body
  const { organizationId, userEmail, roleIds } = await request.json();
  
  // Check permission
  const { data: canInvite } = await supabase.rpc('user_has_permission', {
    p_user_id: user.id,
    p_organization_id: organizationId,
    p_permission_code: 'members.invite'
  });
...
### Caching Strategy (Cloudflare Workers Compatible)

**Good News**: The bitwise permission system works perfectly WITHOUT any external caching! The database-level caching in the `computed_permissions` column is sufficient.

#### Why You Don't Need Redis:

1. **Database-Level Caching**: The `computed_permissions` column on `organization_members` already caches the result
2. **Bitwise Performance**: Checking a bit in an integer is microseconds (< 1ms)
3. **Automatic Invalidation**: Triggers automatically clear the cache when needed
4. **Single Query**: Permission checks require only 1 database query

#### Optional: Cloudflare KV Caching (if you want extra speed)

If you want to add an optional caching layer using Cloudflare KV:

```typescript
// lib/cache/permissions.ts
// This is OPTIONAL - the system works great without it!
...
export async function getCachedPermissions(
  ...
  try {
    ...
    const cached = await env.PERMISSIONS_KV.get(key, 'json');
    return cached as Record<string, boolean> | null;
  } catch (error) {
    console.error('KV cache read error:', error);
    return null; // Fail gracefully, just fetch from DB
  }
}
...
export async function invalidatePermissionCache(
  ...
  try {
    const key = `perms:${userId}:${orgId}`;
    await env.PERMISSIONS_KV.delete(key);
  } catch (error) {
    console.error('KV cache delete error:', error);
  }
}

// Note: KV doesn't support pattern matching like Redis
// For organization-wide invalidation, you'd need to track keys separately
// But honestly, the DB cache is probably enough!
```

#### Recommended Approach for Cloudflare Workers:

**Just use the database cache!** Here's why:

```typescript
// lib/supabase/permissions.ts
// Simple, fast, no external cache needed
...
export async function hasPermission(
  ...
  // This is fast! Single query, uses computed_permissions cache
  const { data, error } = await supabase.rpc('user_has_permission', {
    p_user_id: user.id,
    p_organization_id: organizationId,
    p_permission_code: permissionCode
...
// Average response time: 5-15ms (plenty fast!)
// No need for Redis/KV unless you're doing millions of checks per second
```
...
|----------|---------|------------|------|
| **DB Cache Only** (Recommended) | 5-15ms | Low | Free |
| DB Cache + Cloudflare KV | 1-5ms | Medium | ~$0.50/million reads |
| No Cache (direct calculation) | 20-50ms | Low | Free |

**Recommendation**: Start with DB cache only. Add KV only if you're seeing performance issues (unlikely).
...
✅ **Cloudflare Workers Compatible**: No Redis required!

### Important Note on Caching:

**The bitwise permission system works perfectly WITHOUT Redis or any external cache!**

The system has **built-in caching** via:
1. `computed_permissions` column in `organization_members` (database-level cache)
2. Automatic invalidation triggers
3. 5-minute staleness check (recomputes if needed)

**Performance**: 5-15ms per permission check - fast enough for 99% of applications!

**When to add external caching** (Cloudflare KV, Redis, etc.):
- You're doing 100,000+ permission checks per second
- You need sub-5ms response times
- You have specific edge caching requirements

For most SaaS applications, the database cache is sufficient and much simpler to maintain.
